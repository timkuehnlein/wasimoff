...
export function printPeerStoreUpdates(node: Libp2p, event: keyof Libp2pEvents = "peer:update") {
  node.addEventListener(event, async () => {
    console.log("\x1b[1;35m[PEERSTORE]\x1b[0m", (await node.peerStore.all()).reduce((acc, peer) => Object.assign(acc, {
      [peer.id.toString()]: {
        ...
        metadata: Array.from(peer.metadata.entries()).reduce((acc, [k, v]) => Object.assign(acc, { [k]: "0x" + v.toString() }), { }),
